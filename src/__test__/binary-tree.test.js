...
describe('POST-ORDER', () => {
  test('Expect a string of visited nodes as 8, 9, 7, 6, 2, 4, 5, 3, 1, ', () => {
    let str = '';
...
describe('IN-ORDER', () => {
  test('Expect a string of visited nodes as 8, 9, 7, 6, 2, 1, 3, 4, 5, ', () => {
    let str = '';
    inOrderTraversal(binaryTree.root, (nodeValue) => {
      str += `${nodeValue}, `;
    });
    expect(str.trim()).toEqual('8, 9, 7, 6, 2, 1, 3, 4, 5,');
  });
});
